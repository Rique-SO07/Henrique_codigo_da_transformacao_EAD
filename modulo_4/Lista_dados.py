...
aluno = (
    {}
)  # Dicionário (Chave:Valor), ele é  ideal para mapear propriedades de um objeto
contatos = (
    {}
)  # Dicionário, esse também é ideal para buscas rápidas por chave única (Nome)
...
while True:
    ...
    if opcao == "1":
        ...
        ).strip()  # Atribuição direta de valores às chaves do dicionário
        ...
        for (
            ...
            aluno.items()
        ):  # Aqui o .items() permite iterar simultaneamente sobre a Chave e o Valor
            print(f"{chave}: {valor}")
            print("-" * 25)
        ...
        for i in range(
            1, limite + 1
        ):  # O range(início, fim+1) gera a sequência numérica desejada
        ...
        if sub == "1":  # Variavel para o mini sistema de remoção e adição de contatos
            ...
            contatos[nome] = (
                tel  # Adição/Atualização: se a chave já existe, o valor é atualizado
            )
            ...
            if (
                nome_remover in contatos
            ):  # Busca de Chave: 'in' verifica se a chave existe dentro do meu dicionário, meio confuso
                confirmar = input(
                    f"Tem certeza que deseja remover {nome_remover}? (S/N): "
                ).upper()
                if confirmar == "S":
                    del contatos[
                        nome_remover
                    ]  # o del remove a entrada completa (Chave e Valor) do dicionário
            ...
            sub == "4"
        ):  # Aqui fiz uma alteração, consegui fazer uma ferramenta de buscar contatos já salvos, caso tenha muitos contatos.
